...
{
    // Low-pass / High-pass
    static constexpr T lp_hp_min = std::sqrt(T{0.5});
...
{
    return calc_Q(res, Q_limits<T>::lp_hp_min, Q_limits<T>::lp_hp_max);
}
    ...
    -> coefficients<T>
{
    ...
    T const b1 = calc_b1(phi, b2);
    T const a0 = T{.5} * (T{1} - b2);
    return coefficients<T>{.b2 = b2, .b1 = b1, .a0 = a0, .a1 = T{}, .a2 = -a0};
}
