...
public:
    ...
    event_converter_processor(
        ...
        , m_output_event_ports(
              make_event_ports<outputs_t>(output_names, output_index_sequence))
    {
    }
...
private:
        ...
        m_last = lockstep_events(
            [&]<class... Args>(std::size_t offset, Args&&... args) {
                send_result(
                    ...
                    offset,
                    std::tuple{
                        std::invoke(m_out_gen, std::forward<Args>(args)...)});
            },
            m_last,
            in_bufs);
    }
    ...
    {
        return std::forward_as_tuple(
            ctx.event_outputs.get<std::tuple_element_t<I, outputs_t>>(I)...);
    }
