...
NetworkSettings::setupCallbacks()
{
    if (m_impl->networkController)
    {
        ...
        setAutoDisableOnRecord(
            m_impl->networkController->auto_disable_on_record());
    }

    if (m_impl->wifiManager)
    {
        m_impl->wifiManager->set_scan_complete_callback(
            [this](std::vector<network_manager::wifi_network> const& networks) {
        ...
        m_impl->wifiManager->set_connection_changed_callback(
            [this](
                ...
                {
                    setCurrentSsid(QString::fromStdString(network->ssid));
    ...
    // NFS Client callbacks
    if (m_impl->nfsClient)
    {
        m_impl->nfsClient->set_mount_status_callback(
            [this](std::string const& id, network_manager::nfs_mount_status status) {
        ...
        m_impl->nfsClient->set_mount_error_callback(
            [this](std::string const& id, std::string const& error) {
                auto state = m_impl->nfsClient->get_mount_state(id);
                m_impl->nfsMountModel->updateState(id, state);
                emit nfsMountError(
                    QString::fromStdString(id),
...
NetworkSettings::updateWifiStatus()
{
    ...
    if (connected)
    {
        ...
        setIpAddress(
            QString::fromStdString(m_impl->wifiManager->ip_address()));
        // Signal strength would need to be queried from current connection
    }
    else
    {
        setCurrentSsid(QString());
...
NetworkSettings::updateNfsServerStatus()
{
    ...
    setNfsExportPath(QString::fromStdString(m_impl->nfsServer->export_path()));
    setNfsMountCommand(QString::fromStdString(m_impl->nfsServer->mount_command()));
...
bool
NetworkSettings::testNfsConnection(QString const& serverHost, QString const& remotePath)
{
    if (m_impl->nfsClient)
    {
...
NetworkSettings::getNfsServerExports(QString const& serverHost)
{
    ...
    if (m_impl->nfsClient)
    {
        auto exports = m_impl->nfsClient->get_server_exports(serverHost.toStdString());
...
NetworkSettings::refreshStatus()
{
    ...
    // Update available networks from cache
    if (m_impl->wifiManager)
    {
